...
const hotelControlador = (servicioHotel) => {  
    ...
    const crearHotel = async (req, res) => {
        try {
            const { ciudad_id, nombre, direccion, estrellas, telefono } = req.body;
            const nuevoHotel = { ciudad_id, nombre, direccion, estrellas, telefono };
            const resultado = await servicioHotel.crearHotel(nuevoHotel);
            res.status(201).json({ message: "Hotel creado exitosamente", hotelID: resultado.insertId || resultado });
        } catch (error) {
    ...
    const actualizarHotel = async (req, res) => {
        try {
            const { nombre, direccion, estrellas, telefono, id } = req.body;
            const actualizarHotel = { nombre, direccion, estrellas, telefono, id };
            const resultado = await servicioHotel.actualizarHotel(actualizarHotel);
            res.status(200).json({ message: "Hotel actualizado exitosamente", affectedRows: resultado.affectedRows || resultado });
        } catch (error) {
            res.status(error.status || 500).json({ error: error.message });
        }
    };
    ...
    const actualizarCiudadIdHotel = async (req, res) => {
        try {
            ...
            const resultado = await servicioHotel.actualizarCiudadIdHotel(actualizarCiudadH);
            res.status(200).json({ affectedRows: resultado.affectedRows || resultado });
        } catch (error) {
            res.status(error.status || 500).json({ error: error.message });
        }
    };
